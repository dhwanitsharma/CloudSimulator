...
object Iaas{
  ...
  def Start() :Unit = {
    ...
    val virtualMachine1 = (1 to vm1).map{i=>createVm(i,1)}.toList
    ...
    val virtualMachine2 = (1 to vm2).map{i=>createVm(i,2)}.toList
    ...
    val virtualMachine3 = (1 to vm3).map{i=>createVm(i,2)}.toList
  ...
  def createDatacenter(cloudSim : CloudSim,datacenterNumber : Int) : Datacenter = {
    ...
    val costPerMem = config.getString("Iaas.CloudProviderProperties.datacenter"+datacenterNumber+".cpm").toDouble
    val costPerStorage = config.getString("Iaas.CloudProviderProperties.datacenter"+datacenterNumber+".cps").toDouble
    val costPerBw = config.getString("Iaas.CloudProviderProperties.datacenter"+datacenterNumber+".cpb").toDouble
    val schedulingInterval = config.getString("Iaas.CloudProviderProperties.datacenter"+datacenterNumber+".scheduling_interval").toInt
  ...
  def createHost(host_number : Int ) ={
    ...
    val hostRam = config.getString("Iaas.CloudProviderProperties.host"+host_number+".RAMInMBs").toInt
  ...
  def createVm(vm_Number : Int,dc_number : Int) : Vm ={
    ...
    val virtualMachine_Ram = config.getString("Iaas.CloudProviderProperties.vm"+vm_Number+"-dc"+dc_number+".RAMInMBs").toInt
    val virtualMachine_Bw = config.getString("Iaas.CloudProviderProperties.vm"+vm_Number+"-dc"+dc_number+".BandwidthInMBps").toInt
    val virtualMachine_Size = config.getString("Iaas.CloudProviderProperties.vm"+vm_Number+"-dc"+dc_number+".StorageInMBs").toInt
  ...
  def createCloudlets(cloudLetNumber : Int) : List[Cloudlet] = {
    ...
    val cloudletNumber = config.getString("Iaas.BrokerProperties.cloudlet"+cloudLetNumber+".number").toInt
    ...
    def create(number:Int, model:UtilizationModelDynamic, list : ListBuffer[Cloudlet]) : Unit ={
      if(number == 0) return
      val cloudlet_Pes = config.getString("Iaas.BrokerProperties.cloudlet"+cloudLetNumber+".pes").toInt
      val cloudlet_Size = config.getString("Iaas.BrokerProperties.cloudlet"+cloudLetNumber+".size").toInt
